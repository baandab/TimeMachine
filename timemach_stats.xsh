...
ConvertDatesToReadable() {
    while IFS= read -r line; do
        # The -j flag correctly parses the date string including its timezone (PST/PDT)
        # The +%Y-%m-%d-%H:%M:%S output format will be in the system's current local time.
        date -j -f "%a %b %d %T %Z %Y" "$line" "+%Y-%m-%d-%H:%M:%S" 2>/dev/null
    done
}
...
ConvertDatesToSeconds() {
    while IFS= read -r line; do
        ...
        date -j -f "%a %b %d %T %Z %Y" "$line" "+%s" 2>/dev/null
    done
}
...
TMSnapShots_ByDest() {
    #
    # Iterates through each configured Time Machine destination to report
    # its individual backup status and history.
    #
    BOOTSTRAP_DEST=""
    BOOTSTRAP_SECS=0
    ...
    if [ "$NUM_DESTINATIONS" != 1 ]; then
        ...
        while [ "$X" -lt "$NUM_DESTINATIONS" ]; do
            ...
            SECONDS_NOW=$(date +%s)
            LAST_SNAPSHOT_RELATIVE=$(($SECONDS_NOW - $LAST_SNAPSHOT_SECONDS))

            THIS_SNAPSHOT_COUNT=$(echo "$SNAPSHOTS" | wc -l | sed 's/^ *//g')

            # If a backup hasn't happened in over 2 hours (7200s), identify the destination
            # that has gone the longest without a backup as a candidate for a forced "bootstrap" backup.
            ...
            # For multi-destination setups, this calculates the time difference between the latest
            # snapshot on this drive and the latest snapshot on the *previous* drive in the list.
            if [ "$X" != 0 ]; then
                ((SS_TIME_DIFF = PRIOR_SNAPSHOT_SECONDS - LAST_SNAPSHOT_SECONDS))

                SS_LAST_BACKUP=$(ConvertSecs_to_HumanReadable "$SS_TIME_DIFF")

                SS_TIME_DIFF="($SS_LAST_BACKUP difference)"
            else
                SS_TIME_DIFF=""
            fi
        ...
        # If the -b (bootstrap) flag is used and a backup is not currently running,
        # start a backup on the destination that was identified as being the most "out of date".
        if [ "$BOOTSTRAP" = 1 ] && [ "$BOOTSTRAP_DEST" != "" ] && [ $(/usr/bin/tmutil currentphase) = "BackupNotRunning" ]; then
            BOOTSTRAP_SECS=$(ConvertSecs_to_HumanReadable "$BOOTSTRAP_SECS")
    ...
    if [ -e /usr/bin/tmutil ]; then
        ...
        if [ -z "$V" ]; then
            ...
            if [ "$TM_SNAPSHOTS" == "" ]; then
                ...
                ((TM_ELAPSED = $CURRENT_SEC - $LAST_SNAPSHOT_SEC))
                TM_DAYS=$(($TM_ELAPSED / 3600 / 24))
                TIME_AGO=$(ConvertSecs_to_HumanReadable "$TM_ELAPSED")
...
while [ 1 = 1 ]; do
    ...
    # If the -p flag is used, query the system log for recent Time Machine
    # activity to find the path of the file currently being backed up.
    if [ "$SHOW_PATH" = "1" ]; then
        LOG_START="$(date -j -v-10M +'%Y-%m-%d %H:%M:%S')"
    ...
    # To avoid spamming the log, only print the file path if it has changed since the last check.
    ...
    # This counter tracks how many check cycles the backup has been in the same phase.
    # It can be useful for seeing if a backup is "stuck" on a certain step.
    ...
    "Copying")
        ...
        # The following section calculates the progress of the "Copying" phase, including
        # percentage complete, number of files remaining, and MB remaining to be copied.
        ...
        # Sometimes tmutil reports more files copied than total, which results in a negative
        # number. This logic handles that case by changing the description.
        if [ $REM_FILES -lt 0 ]; then
            REM_FILES=$(echo "-1 * $REM_FILES" | bc)
    ...
    "ThinningPostBackup")
        ...
        ;;
    *)
        printf "$(Print_DateTime)- $MY_HOSTNAME - $NAME - Time Machine Status ($INFO_TEXT): [$PHASE_COUNT] unknown phase \n" | tee -a "$LOGFILE"
        ;;
    esac
